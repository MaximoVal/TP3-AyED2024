...
#--------------------------------------------------------------------------VARIABLES-----------------------------------------------------------------------------------------------------
# ENTEROS:cont,opc,op,opc2,opc3,oped,oped1,opb,eleccion,opce,opgu,opgu2,num,verificacion,nmatch,opu,contlike,totmatch,contmismatch,contnomatch,porcent,dado,recibido,cantest,contreport,cantmod,candidato,idmoderador,idestudiante,file,film,filasE,filasM,columE,columM,busq,busqlike,busqid,fillike,filid,FILA,reportante,reportado,like,matcheos_posibles
...
def busquedaid(A,ID):
      ...
      while A[filid][8]!= str(ID) and filid<(cantest-1):
            filid=filid+1
...
def reportar_candidato(R):
      ...
      while identificador_candidato==str(file):
            print('No te puedes autoreportar')
            identificador_candidato = input("Ingresar el ID de un usuario a reportar:")
      ...
      if candidato!=-1:
            R[contreport][0]=file
            R[contreport][1]=filid
            R[contreport][2]=motivo
            R[contreport][3]='0'
            ...
            match num:
                  case 1:     
                              ...
                              if R[0][0]=='':
                                    ...
                                    print('Hay '+str(contreport)+' reportes efectuados')
                                    for i in range (contreport):
                                        print('Reporte ',i+1)
                                        reportante=R[i][0]
                                        reportado=R[i][1]
                                        eportante=int(reportante)
                                        reportado=int(reportado)
                                        if E[reportado][2]=='Activo' and E[reportante][2]=='Activo':
                                                if R [i][3]=='0':   
                                                      print('Reporte efectuado por usuario de ID:',R[i][0])
                                                      print('Reporte dirigido hacia el usuario de ID:',R[i][1])
                                                      print('Motivo del reporte:',R[i][2])
                                        ...
                                        elif E[reportado][2]!='Activo':
                                            print('El estudiante al que se quiere reportar esta inactivo')
                                        elif E[reportante][2]!='Activo':
                                            print('El estudiante que reporta esta inactivo')
